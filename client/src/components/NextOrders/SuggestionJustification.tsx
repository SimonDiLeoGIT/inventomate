  ...
  return (
    <>
      ...
      >
        ...
        <p className="p-2 col-span-2">{order.stock_actual}</p>
        <p className="p-2 col-span-2">{order.cantidad_a_comprar}</p>
      </li>
      ...
      <aside className={`fixed w-screen h-screen overflow-hidden top-0 left-0 ${!isOpen && 'hidden'} opacity-animation grid place-content-center`}>
        <section className='-bg--color-white p-4 relative w-screen max-w-xl shadow-lg -shadow--color-black-shadow rounded-lg'>
          ...
          <ul className="grid grid-cols-3 py-2">
            ...
            <li className=""><span className="mr-2 font-semibold -text--color-mate-dark-violet">Actual Stock:</span> {order.stock_actual}</li>
            <li className=""><span className="mr-2 font-semibold -text--color-mate-dark-violet">Quantity to Order: </span>{order.cantidad_a_comprar}</li>
          </ul>
