  ...
  return (
    <>
      ...
      <aside className={`fixed w-screen h-screen overflow-hidden top-0 left-0 ${!isOpen && 'hidden'} opacity-animation grid place-content-center`}>
        <section className='-bg--color-white p-4 relative'>
          <header>
          ...
          <form className="relative" onSubmit={handleSubmit}>
                  ...
                  roles?.map((role) => {
                    return (
                      <li className="p-2 flex items-center -bg--color-border-very-lightest-grey rounded-lg w-full">
                        <p>{role.nombreRol}</p>
                        <input type="checkbox" value={role.idRol} className="m-auto mr-0 hover:cursor-pointer" onChange={handleRoleSelect} />
                      </li>
                    )
                  })
                }
            ...
            <button type="submit" className="-bg--color-semidark-violet font-bold -text--color-white w-full mt-4 p-2 rounded-lg hover:opacity-80">Confirm</button>
          </form>
        </section>
      </aside>
    </>
